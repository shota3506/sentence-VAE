...
def main():
    ...
    for epoch in range(args.num_epochs):
        ...
        for itr, s in enumerate(pbar):
            ...
            kl_loss = -0.5 * torch.mean(torch.sum(1 + logvar - mean.pow(2) - logvar.exp(), dim=-1))
        ...
        logger.info('[Epoch %d/%d] Training loss: %.2f, CE loss: %.2f, KL loss: %.2f, Validation loss: %.2f, CE loss: %.2f, KL loss: %.2f' % (
            epoch, args.num_epochs, 
            train_loss, train_ce_loss, train_kl_loss,
            valid_loss, valid_ce_loss, valid_kl_loss,
        ))
