...
class BeamSearch:
    ...
    for decoding the most likely sequences.
    ...
    def search(
        ...
        to find themost likely target sequences.
        Notes
        -----
        ...
        with finite log probability (non-zero probability) returned
        by the step function.
        Therefore if you're using a mask you may want to check the results from
        `search` and potentially discard sequences
        with non-finite log probability.
        # Parameters
        start_predictions : `torch.Tensor`
            A tensor containing the initial predictions with shape
            `(batch_size,)`.
            Usually the initial predictions are just the index of the
            "start" tokenin the target vocabulary.
        start_state : `StateType`
        ...
        step : `StepFunctionType`
            ...
            time step. The function should accept two arguments.
            The first being a tensor of shape `(group_size,)`,
            representing the index of the predicted tokens from the last time
            step, and the second being the current state.
            The `group_size` will be `batch_size * beam_size`,
            except in the initial step, for which it will just be `batch_size`.
            The function is expected to return a tuple, where the first element
            is a tensor of shape `(group_size, target_vocab_size)` containing
            the log probabilities of the tokens for the next step,
            and the second element is the updated state.
            The tensor in the state should have shape
        ...
        # within the main loop we are going from the `beam_size` elements of
        # the beam to `beam_size`^2 candidates from which we will select the
        # top `beam_size` elements for the next iteration.
        # shape: (batch_size, num_classes)
        start_class_log_probabilities, state = step(
            start_predictions, start_state, 0
        )
        ...
        if (
            self.beam_size == 1
            and (start_predicted_classes == self.end_index).all()
        ):
            warnings.warn(
                "Empty sequences predicted. "
                "You may want to increase the beam size or ensure "
        ...
        for timestep in range(self.max_steps - 1):
            ...
            class_log_probabilities, state = step(
                last_predictions, state, timestep + 1
            )
            ...
            # Here we expand the last log probabilities to
            # (batch_size * beam_size, per_node_beam_size)
            # so that we can add them to the current log probs
            # for this timestep.
            # This lets us maintain the log probability
            # of each element on the beam.
            ...
            # The beam indices come from a
            # `beam_size * per_node_beam_size` dimension where the indices
            # with a common ancestor are grouped together.
            # Hence dividing by per_node_beam_size gives the ancestor.
            # shape: (batch_size, beam_size)
...
class DiverseBeamSearch:
    ...
    def search(
        ...
        for timestep in range(self.max_steps - 1):
            ...
            # shape: (batch_size, beam_size * per_node_beam_size)
            reshaped_rewritten_summed = (
                rewritten_summed_top_log_probabilities.reshape(
                    batch_size, self.beam_size * self.per_node_beam_size
                )
            )
        ...
        if not torch.isfinite(last_log_probabilities).all():
            warnings.warn(
                ...
                "This can happen when the beam size is larger "
                "than the number of valid (non-zero "
...
class RandomSample:
    ...
    def search(
        ...
        # List of (batch_size, sample_size) tensors. One for each time step.
        # Does not include the start symbols, which are implicit.
        predictions: List[torch.Tensor] = []
        ...
        # within the main loop we are going from the `sample_size` elements of
        # the beam to `sample_size`^2 candidates from which we will select the
        # top `sample_size` elements for the next iteration.
        # shape: (batch_size, num_classes)
        start_class_log_probabilities, state = step(
            start_predictions, start_state, 0
        )
        ...
        truncated_start_class_log_probabilities, truncated_classes = (
            torch.topk(start_class_log_probabilities, self.k)
        )
        ...
        for timestep in range(self.max_steps - 1):
            ...
            # Here we are finding any samples where we predicted the end token
            # in the previous timestep and replacing the distribution with a
            # one-hot distribution, forcing the beam to predict the end token
            # this timestep as well.
            # shape: (batch_size * sample_size, num_classes)
            cleaned_log_probabilities = torch.where(
            ...
            # shape: (batch_size, sample_size)
            truncated_cleaned_log_probabilities, truncated_classes = (
                torch.topk(cleaned_log_probabilities, self.k)
            )
            truncated_predicted_classes = torch.multinomial(
                torch.exp(truncated_cleaned_log_probabilities),
                1,
                replacement=True,
            )
            ...
            predicted_classes = predicted_classes.reshape(
                batch_size, self.sample_size
            )
            ...
            last_log_probabilities = (
                top_log_probabilities + last_log_probabilities
            )
